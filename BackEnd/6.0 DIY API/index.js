...
app.get("/filter", (req, res) => {
  ...
  const filteredJoke = jokes.filter(
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  // Find the joke index in the array
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  // Get the existing joke
  const existingJoke = jokes[searchIndex];
  // Update only the fields that the user provided
  const updatedJoke = {
    id: existingJoke.id, // Keep the same ID
    jokeText: req.body.text || existingJoke.jokeText, // If text is provided, update it, otherwise keep the old value
    jokeType: req.body.type || existingJoke.jokeType, // If type is provided, update it, otherwise keep the old value
  };
  // Replace the joke in the array with the updated one
...
app.delete("/jokes/all", (req, res) => {
  ...
  if (securityKey === masterKey) {
    ...
    res.status(404).json(`You are not authorised to perform this action.`);
  }
});
